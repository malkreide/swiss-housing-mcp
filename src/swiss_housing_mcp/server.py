...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "Six tools read a cantonal dump that is fetched on first use, so the "
        "first call for a canton is slow and later ones are not: "
        "`lookup_dwellings`, `new_construction`, `construction_pipeline`, "
        "`buildings_in_bbox`, `municipality_housing_stats` and `explain_code`. "
        "`dump_status` is NOT one of them: it reports cache state only and "
        "never contacts the source, which is exactly why it stays usable "
        "during an upstream outage — it never returns a silently empty "
        "record.\n\n"
        "Which canton those six read is not uniform, and the wrong one "
        "returns empty or wrong data rather than an error:\n"
        "- `new_construction`, `construction_pipeline` and "
        "`municipality_housing_stats` resolve the canton themselves from a "
        "BFS municipality number (261 = City of Zurich). That lookup only "
        "searches dumps already cached on disk, plus a seed covering 261, so "
        "with an empty cache directory it fails for every other municipality. "
        "The cache is a directory, not process state: a dump fetched by an "
        "earlier run still counts after a restart. Pass `canton` whenever you "
        "know it instead of waiting for that failure.\n"
        "- `lookup_dwellings`, `buildings_in_bbox` and `explain_code` resolve "
        "nothing and default to `zh`. ALWAYS pass `canton` explicitly for "
        "anything outside the canton of Zurich — an EGID or bounding box from "
        "another canton is otherwise looked up in the Zurich dump, which "

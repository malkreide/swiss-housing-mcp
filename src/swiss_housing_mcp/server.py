...
mcp = MCPServer(
    ...
    instructions=(
        ...
        "The remaining tools read a cantonal dump that is fetched on first "
        "use, so the first call for a canton is slow and later ones are not. "
        "Which canton they read is NOT uniform, and picking the wrong one "
        "returns empty or wrong data rather than an error:\n\n"
        "- `new_construction`, `construction_pipeline` and "
        "`municipality_housing_stats` take a municipality by its BFS number "
        "(261 = City of Zurich) and resolve the canton themselves. Pass "
        "`canton` only if that resolution fails.\n"
